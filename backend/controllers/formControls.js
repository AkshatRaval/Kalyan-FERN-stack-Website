...
export const postAForm = async (req, res) => {
  ...
  try {
    // Parse incoming text fields safely
    ...
    // Safe destructuring (avoid crash if teamInfo is null)
    ...
    // Files (these assume multer.diskStorage so file.path exists)
    const photoFile = req.files?.photo?.[0];
    const idProofFile = req.files?.idProof?.[0];
    const academicRecordsFile = req.files?.academicRecords?.[0];

    // === Parallelize uploads: do all file uploads concurrently ===
    ...
    // Wait for all three uploads at once (saves wall-clock time)
    const [photoLinks, idProofLinks, academicRecordsLinks] = await Promise.all(uploadPromises);

    // Prepare finalData (use links from uploads)
    const finalData = {
    ...
    // Generate app id and PDF (pdfLocalPath is the outer var)
    const appId = getNextAppId(formName);
    pdfLocalPath = await makePdf(finalData, appId); // assign, not redeclare

    // Upload PDF (can be done while writing to DB, but we'll upload now)

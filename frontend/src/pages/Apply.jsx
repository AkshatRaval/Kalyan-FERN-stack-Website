...
import { Calendar, Users, Clock, MapPin, FileText, ExternalLink } from 'lucide-react';
...
const getStatusBadge = (status) => {
  switch (status) {
    case 'open':
      return (
        <div className="bg-green-100 text-green-800 border border-green-200 text-xs px-3 py-0.5 rounded-2xl font-medium">
          Registration Open
        </div>
      );
    case 'coming-soon':
      return (
        <div className="bg-gray-100 text-gray-800 border border-gray-200 text-xs px-3 py-0.5 rounded-2xl font-medium">
          Coming Soon
        </div>
      );
    case 'closed':
      return (
        <div className="bg-red-100 text-red-800 border border-red-200 text-xs px-3 py-0.5 rounded-2xl font-medium">
          Registration Closed
        </div>
      );
    default:
      return <div className="text-xs px-3 py-0.5 rounded-2xl border">{status}</div>;
  }
};
...
const getApplicationStatusBadge = (status) => {
  switch (status) {
    case 'pending':
      return <div className="bg-gray-100 text-gray-800 text-xs px-3 py-0.5 rounded-2xl">Under Review</div>;
    case 'approved':
      return <div className="bg-green-100 text-green-800 text-xs px-3 py-0.5 rounded-2xl">Approved</div>;
    case 'rejected':
      return <div className="bg-red-100 text-red-800 text-xs px-3 py-0.5 rounded-2xl">Rejected</div>;
    default:
      return <div className="text-xs px-3 py-0.5 rounded-2xl border">{status}</div>;
  }
};
...
const Apply = () => {
  ...
  // 2. Sort: open → coming-soon → closed (open forms always at the top)
  const filteredActivities = (cat) => {
    const list = cat === 'all'
      ? [...activities]
    ...
    return list.sort((a, b) => {
      ...
      // Within same status: pinned first, then original array order
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative py-24 lg:py-40 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(120,119,198,0.1),transparent_50%),radial-gradient(circle_at_30%_80%,rgba(120,119,198,0.08),transparent_50%)]" />
        <div className="absolute top-32 left-20 w-40 h-40 bg-gradient-to-br from-primary/10 to-transparent rounded-full blur-3xl" />
        <div className="absolute bottom-32 right-20 w-60 h-60 bg-gradient-to-br from-secondary/10 to-transparent rounded-full blur-3xl" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center z-10">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: 'easeOut' }}>
            ...
            >
              ...
              designed to{' '}
              <span className="text-primary font-medium">nurture talent and recognise excellence</span>.
            </motion.p>
          </motion.div>
        </div>
        ...
        />
        <motion.div
          animate={{ y: [0, 20, 0], rotate: [360, 180, 0], scale: [1, 0.9, 1] }}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="text-center">
                <h2 className="text-3xl lg:text-5xl font-bold mb-4">My Applications</h2>
                <p className="text-lg text-muted-foreground">Track the status of your submitted applications</p>
              </div>

              {userApplicationsData && userApplicationsData.length > 0 ? (
                        ...
                        <CardContent className="p-6">
                          <div className="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0">
                            ...
                            <div className="flex items-center space-x-4">
                              ...
                              <button
                                className="flex items-center border px-3 p-2 hover:bg-secondary/50 rounded-xl cursor-pointer transition-all text-sm"
                                onClick={() => navigate(`/application/${application.formType}/${application.id}`)}
                              >
      ...
      <section className="min-h-screen py-20">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 w-fit mx-auto gap-2 sm:gap-5 bg-gray-100 dark:bg-gray-800 p-2 rounded-2xl shadow-md">
            {filters.map((filter) => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm sm:text-base font-medium transition-colors duration-300 ease-in-out ${
        ...
        {/* Cards — open forms always appear first */}
          ...
          {filteredActivities(category).map((activity, index) => (
            ...
            >
              <Card className={`h-full border-none shadow-lg hover:shadow-xl transition-shadow duration-300 mx-4 ${
                activity.status === 'open' ? 'ring-1 ring-green-200 dark:ring-green-900' : ''
              }`}>
                <div className="relative">
                  ...
                  {/* Optional "Open" highlight banner */}
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-baseline gap-1">
                        <p className="font-semibold text-lg text-primary">
                          {activity.fee === 0 ? 'Free To Apply' : `₹${activity.fee}`}
                        </p>
                    ...
                    <div className="flex space-x-3 items-center text-sm">
                      <button
                        className="border border-border flex items-center px-2 py-1 rounded-sm hover:bg-accent transition-colors"
                        type="button"
                        onClick={() => { setSelectedActivity(activity); setIsDetailsModalOpen(true); }}
                      >
                      ...
                      {activity.status === 'open' && (
                        <Link
                          className="bg-primary px-2 py-1 border rounded-sm text-primary-foreground hover:bg-primary/90 transition-colors"
                          to={`/application/${activity.link}`}
                        >
                          Apply Now
                        </Link>
                      )}
      ...
      <section className="py-20 bg-gradient-to-r from-primary to-primary/90 text-primary-foreground">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div
            ...
            className="space-y-8"
          >
            <h2 className="text-3xl lg:text-5xl font-bold">Ready to Participate?</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {!currentUser ? (
                <button
                  className="border border-primary-foreground rounded-lg p-3 px-6 font-semibold"
